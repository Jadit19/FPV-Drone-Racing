...
void imageCb(const sensor_msgs::ImageConstPtr& msg){
    ...
    cv::inRange(cv_ptr->image, cv::Scalar(0, 0, 100), cv::Scalar(50, 50, 255), red_filtered);
    cv::GaussianBlur(red_filtered, red_filtered, cv::Size(5, 5), 0);
    ...
    cv::Mat drawing = cv::Mat::zeros(red_filtered.size(), CV_8UC3);
